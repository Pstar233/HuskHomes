...
public class SqLiteDatabase extends Database {
        ...
        getHome(home.getUuid()).ifPresentOrElse(presentHome -> {
            try {
                ...
                plugin.log(Level.SEVERE,
                        "无法更新数据库中的家庭 " + home.getOwner().getName(), e);
            }
        }, () -> {
            try (PreparedStatement statement = getConnection().prepareStatement(format("""
                ...
                plugin.log(Level.SEVERE, "未能设定回家时间 " + home.getOwner().getName(), e);
            }
        });
    }
    ...
    /**
     * 从 SQLite 数据库迁移数据到目标数据库（如 MySQL）
     *
     * @param database 目标数据库实例
     */
    public void migrateData(@NotNull Database database) {
        plugin.log(Level.INFO, "开始从 SQLite 迁移数据到目标数据库...");

        int playerCount = 0;
        int homeCount = 0;
        int warpCount = 0;
        final java.util.concurrent.atomic.AtomicInteger cooldownCount = new java.util.concurrent.atomic.AtomicInteger(0);

        try {
            //收集所有玩家数据
            final List<RawUserData> userDataList = new ArrayList<>();
            try (PreparedStatement ps = getConnection().prepareStatement(format("""
                    SELECT `uuid`, `username`, `home_slots`, `ignoring_requests`,
                           `last_position`, `offline_position`, `respawn_position`
                    FROM `%player_data%`"""))) {
                final ResultSet rs = ps.executeQuery();
                while (rs.next()) {
                    final UUID uuid = UUID.fromString(rs.getString("uuid"));
                    final String username = rs.getString("username");
                    final int homeSlots = rs.getInt("home_slots");
                    final boolean ignoringRequests = rs.getBoolean("ignoring_requests");

                    int lastPosId = rs.getInt("last_position");
                    final boolean hasLastPos = !rs.wasNull();
                    int offlinePosId = rs.getInt("offline_position");
                    final boolean hasOfflinePos = !rs.wasNull();
                    int respawnPosId = rs.getInt("respawn_position");
                    final boolean hasRespawnPos = !rs.wasNull();

                    userDataList.add(new RawUserData(uuid, username, homeSlots, ignoringRequests,
                            hasLastPos ? lastPosId : -1,
                            hasOfflinePos ? offlinePosId : -1,
                            hasRespawnPos ? respawnPosId : -1));
                }
            }

            //迁移玩家及其 homes
            for (final RawUserData data : userDataList) {
                final User user = User.of(data.uuid, data.username);
                database.ensureUser(user);
                database.updateUserData(new SavedUser(user, data.homeSlots, data.ignoringRequests));

                //迁移玩家位置
                if (data.lastPosId >= 0) {
                    readRawPosition(data.lastPosId)
                            .ifPresent(pos -> database.setLastPosition(user, pos));
                }
                if (data.offlinePosId >= 0) {
                    readRawPosition(data.offlinePosId)
                            .ifPresent(pos -> database.setOfflinePosition(user, pos));
                }
                if (data.respawnPosId >= 0) {
                    readRawPosition(data.respawnPosId)
                            .ifPresent(pos -> database.setRespawnPosition(user, pos));
                }

                // 迁移该玩家的 homes
                for (final Home home : getHomes(user)) {
                    database.saveHome(hasNonBmp(home) ? sanitizeHome(home) : home);
                    homeCount++;
                }
            ...
            //迁移冷却时间
            try (PreparedStatement ps = getConnection().prepareStatement(format("""
                    SELECT `player_uuid`, `type`, `end_timestamp`
                    FROM `%player_cooldowns_data%`"""))) {
                final ResultSet rs = ps.executeQuery();
                while (rs.next()) {
                    final UUID uuid = UUID.fromString(rs.getString("player_uuid"));
                    final String typeStr = rs.getString("type");
                    final Instant endTime = rs.getTimestamp("end_timestamp").toInstant();

                    findCooldownAction(typeStr).ifPresent(action -> {
                        final User user = User.of(uuid, "");
                        database.ensureUser(user);
                        database.setCooldown(action, user, endTime);
                        cooldownCount.incrementAndGet();
                    });
                }
            }

            // 传送状态跨服传送中属于瞬时数据，跳过不迁移
            plugin.log(Level.INFO, "数据迁移完成! 玩家: " + playerCount
                                   + ", 家(Home): " + homeCount
                                   + ", 传送点(Warp): " + warpCount
                                   + ", 冷却时间: " + cooldownCount.get());

        } catch (SQLException e) {
            plugin.log(Level.SEVERE, "数据迁移失败", e);
        }
    }

    /**
     * 按 ID 从 SQLite 读取原始位置数据
     */
    @NotNull
    private Optional<Position> readRawPosition(int positionId) {
        try (PreparedStatement ps = getConnection().prepareStatement(format("""
                SELECT `x`, `y`, `z`, `yaw`, `pitch`, `world_name`, `world_uuid`, `server_name`
                FROM `%position_data%`
                WHERE `id` = ?"""))) {
            ...
            if (rs.next()) {
                return Optional.of(Position.at(
                        ...
                        World.from(rs.getString("world_name"),
                                UUID.fromString(rs.getString("world_uuid"))),
                        rs.getString("server_name")));
            }
        } catch (SQLException e) {
            plugin.log(Level.WARNING, "读取位置 ID " + positionId + " 失败", e);
        }
        return Optional.empty();
    }

    /**
     * 根据数据库中的类型字符串查找对应的 Action 枚举
     */
    @NotNull
    private Optional<TransactionResolver.Action> findCooldownAction(@NotNull String typeStr) {
        try {
            return Optional.of(TransactionResolver.Action.valueOf(typeStr.toUpperCase(Locale.ENGLISH)));
        } catch (IllegalArgumentException e) {
            plugin.log(Level.WARNING, "未知的冷却类型: " + typeStr + ", 跳过");
            return Optional.empty();
        }
    }
    ...
    private boolean hasNonBmp(@NotNull Home home) {
        return hasNonBmp(home.getMeta().getName())
                || hasNonBmp(home.getMeta().getDescription());
    }

    private boolean hasNonBmp(@NotNull String s) {
        return s.codePoints().anyMatch(cp -> cp > 0xFFFF);
    }

    /**
     * 将字符串中的非 BMP 字符替换为 {hex} 格式占位符（如 🐟 → {1F41F}），
     * 解决 MySQL utf8 编码不兼容问题，保证占位符唯一不重复
     */
    @NotNull
    private String replaceNonBmp(@NotNull String input) {
        ...
        while (i < input.length()) {
            ...
            if (cp <= 0xFFFF) {
                ...
                sb.append('{').append(Integer.toHexString(cp).toUpperCase(Locale.ENGLISH)).append('}');
            }
    ...
    @NotNull
    private Home sanitizeHome(@NotNull Home home) {
        final String name = home.getMeta().getName();
        final String desc = home.getMeta().getDescription();
        final String cleanName = replaceNonBmp(name);
        final String cleanDesc = replaceNonBmp(desc);
        plugin.log(Level.WARNING, "Home \"" + name + "\" → \"" + cleanName + "\" (剔除特殊字符)");
        return Home.from(
                home.getX(), home.getY(), home.getZ(),
                home.getYaw(), home.getPitch(),
                home.getWorld(), home.getServer(),
                PositionMeta.from(cleanName, cleanDesc,
                        home.getMeta().getCreationTime(),
                        home.getMeta().getSerializedTags()),
                home.getUuid(), home.getOwner(), home.isPublic());
    }

    /**
     * 临时数据容器 - 存储从 SQLite 读取的原始玩家行数据
     */
    private static class RawUserData {
        private final UUID uuid;
        private final String username;
        private final int homeSlots;
        private final boolean ignoringRequests;
        private final int lastPosId;
        private final int offlinePosId;
        private final int respawnPosId;

        private RawUserData(@NotNull UUID uuid, @NotNull String username,
                            int homeSlots, boolean ignoringRequests,
                            int lastPosId, int offlinePosId, int respawnPosId) {
            this.uuid = uuid;
            this.username = username;
            this.homeSlots = homeSlots;
            this.ignoringRequests = ignoringRequests;
            this.lastPosId = lastPosId;
            this.offlinePosId = offlinePosId;
            this.respawnPosId = respawnPosId;
        }
    }
